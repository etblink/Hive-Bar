...
(function attachSocialActions(global) {
  ...
  function formPayload(form) {
    ...
    for (const [name, value] of new FormData(form)) {
      ...
      if (name === 'tags') {
        payload.tags = normalized.split(',').map((tag) => tag.trim()).filter(Boolean);
  ...
  class SocialActionController {
    constructor({ fetchImpl = global.fetch ? global.fetch.bind(global) : null, keychainFactory = () => new global.HiveBarKeychain.KeychainAdapter(), review = null, waitImpl = wait, reload = () => global.location.reload() } = {}) {
    ...
    async run(form) {
      ...
      try {
        ...
        preflight = await this.request(`/api/social/preflight/${action}`, { method: 'POST', csrfToken: session.csrfToken, body: formPayload(form) });
        ...
        const result = await this.keychainFactory().broadcast({ account: signerAccount, operations: preflight.operations, authority: preflight.authority });
        broadcastAccepted = Boolean(result?.accepted);
        const accepted = await this.request(`/api/social/preflight/${preflight.id}/accepted`, { method: 'POST', csrfToken: session.csrfToken, body: { transactionId: result?.transactionId || null } });
        ...
        for (let attempt = 0; attempt < OBSERVATION_ATTEMPTS; attempt += 1) {
          ...
          const observation = await this.request(`/api/social/preflight/${preflight.id}/observe`, { method: 'POST', csrfToken: session.csrfToken });
          setFormStatus(form, observation.message);
          if (observation.state === 'observed') { this.reload(); return; }
        }
      } catch (error) {
        if (preflight && !broadcastAccepted && session?.csrfToken) await this.cancel(preflight.id, session.csrfToken).catch(() => {});
        const prefix = broadcastAccepted ? 'Keychain approved this action, but Hive confirmation is still pending. Don’t try it again yet. ' : '';
    ...
    async cancel(preflightId, csrfToken) {
      return this.request(`/api/social/preflight/${preflightId}/cancel`, { method: 'POST', csrfToken });
    }

    reviewDialog(preflight) {
      const dialog = document.querySelector('[data-social-confirm]');
      if (!dialog || typeof dialog.showModal !== 'function') {
        return Promise.resolve(global.confirm(`Review ${preflight.action} as @${preflight.account} before opening Keychain?\n\nTechnical fingerprint: ${preflight.fingerprint}\n\n${JSON.stringify(preflight.operations, null, 2)}`));
      }
      dialog.querySelector('[data-social-summary]').textContent = Object.entries(preflight.summary).map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(', ') : value}`).join('\n');
      ...
      return new Promise((resolve) => {
        ...
        const finish = (value) => { confirmButton.removeEventListener('click', confirm); cancelButton.removeEventListener('click', cancel); dialog.removeEventListener('cancel', escape); dialog.close(); resolve(value); };
