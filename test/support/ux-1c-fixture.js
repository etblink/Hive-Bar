...
function createUx1cVisualFixture() {
  const config = configFrom({
    HIVE_WRITE_MODE: 'beta',
    HIVE_SIGNER_MODE: 'keychain',
    RATE_LIMIT_MAX: '10000',
    SESSION_SECRET: 'ux-1c-visual-session-secret-that-is-at-least-32-bytes',
  });
  ...
  app.use((req, res, next) => {
    ...
    return res.status(405).json({ error: { code: 'UX_1C_VISUAL_MUTATION_FORBIDDEN' } });
  });
  app.use(application);
  return { account: ACCOUNT, app, config, mutationAttempts, rpcPool, token };
}

module.exports = { createUx1cVisualFixture };
