...
const { V1_ACTIONS, V1_ACTIVE_ACTIONS, V1_POSTING_ACTIONS } = require('../v1/actions');

const V1_EXPLICIT_SETTINGS = Object.freeze([
  'NODE_ENV',
  'HIVE_BAR_HOST',
  'PORT',
  'BIND_HOST',
...
function assertPrivexV1Release(config, source = {}) {
  const issues = [];
  const missing = V1_EXPLICIT_SETTINGS.filter((name) => !hasOwn(source, name));
  const suppliedHost = String(source.HIVE_BAR_HOST || '');
  const publicHost = normalizePublicHost(suppliedHost);

  if (missing.length > 0) {
    issues.push(`explicit V1 decisions are required for ${missing.join(', ')}`);
  }
  ...
  if (config.hive.controlledActions.length !== 0) {
    issues.push('HIVE_CONTROLLED_ACTIONS must be explicitly empty in V1 self-signing mode');
  }
  ...
  if (
    ...
    issues.push('V1 self-signing mode must contain no M9/M10/M12 controlled or delegated posting state');
  }
  ...
  if (issues.length > 0) {
    throw new Error(`Privex V1 release gate failed: ${issues.join('; ')}`);
  }

  return Object.freeze({
    profile: 'privex-v1-self-signing',
    ...
    v1Actions: V1_ACTIONS,
    v1PostingActions: V1_POSTING_ACTIONS,
    v1ActiveActions: V1_ACTIVE_ACTIONS,
...
module.exports = {
  V1_EXPLICIT_SETTINGS,
  assertPrivexV1Release,
};
